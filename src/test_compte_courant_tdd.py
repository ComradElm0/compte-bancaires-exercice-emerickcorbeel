...
# renommer "overderaft_limit" par ta variable de limite d'authorisation de decouvert
    def test_cc_overdraft(self):
        ...
        montant: int = 250
        compte_courant = CompteCourant("username", autorisation_decouvert= -200)
        with pytest.raises(Exception):
            compte_courant.retrait(montant)

    def test_cc_appliquer_agios1(self):
        """regular case : agios applied in case of solde < 0"""
        montant: int = 150
        compte_courant = CompteCourant("username", autorisation_decouvert= -1000)
        compte_courant.retrait(montant)
        assert compte_courant.solde == -165

    def test_cc_appliquer_agios2(self):
        """regular case : agios applied in case of solde < 0"""
        montant: int = 520
        compte_courant = CompteCourant("username", autorisation_decouvert=-1000)
        compte_courant.retrait(montant)
    ...
    def test_cc_appliquer_agios3(self):
        """regular case : agios applied in case of solde < 0"""
        montant: int = 420
        compte_courant = CompteCourant("username", autorisation_decouvert=-1000)
        compte_courant.retrait(montant)
